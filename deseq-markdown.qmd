...
patient_data = pd.read_csv("data/BW_patientdata.csv")
...
###### Batch correcting counts

```{python} 
from inmoose.pycombat import pycombat_seq 
patient_data["Country_Skerget"]
patient_data["PUBLIC_ID"]

#only include patients in both dataframes

rna_seq_df = rna_seq_df[patient_data["PUBLIC_ID"]]

rna_seq_df = pycombat_seq(rna_seq_df.astype("float64"), patient_data["Country_Skerget"])
...
def vol_plot(sigs, label_genes: bool = False, hor_line: int = 2, vert_line_r: int = 1, vert_line_l: int = -1):
  ...
  if label_genes: 
    ...
    for i, v in sigs.iterrows():
        if v["nlog10"] >  5:
            text.append(ax.text(v["log2FoldChange"], v["nlog10"],v["gene_names"], fontsize=6, ha="right"))
